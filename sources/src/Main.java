...
* the WAVE audio file containing the win sound is the Level up sound effect from a royalty free sound library
* Sound File Source: https://pixabay.com/sound-effects/level-up-191997/
...
* if the user preforms two rounds where the art dealer selects every card, the user "wins" the round. when the user wins a round a
* victory message is shown, a sound effect plays, and the program advances to the next pattern in the sequence.
* currently, there are 12 patterns, when the user wins the last pattern a more elaborate victory message plays
* the user can exit the application at any time, discarding any incomplete set of cards. Before the application closes,
* it will open and append every complete set of cards that have been selected during the session to the end of the log file
* included with the current date.
...
shown to the user of the program, as well as a WAVE audio file.
